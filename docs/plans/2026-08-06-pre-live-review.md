...
> - **Atualização 2026-08-11 — itens 4–8 executados:** A5 ✅ (next 15.5.23 + axios 1.19.0 web/backend, form-data, websocket-driver; zero critical; builds+248 testes verdes; highs restantes = major Nest 11/sharp, pós-V1). A4 ✅ (playstore-service-account/keystore/.env.production blindados em gitignore+easignore, validado com a lib ignore do eas-cli). A6 ✅ (EXPO_PUBLIC_SENTRY_DSN criado nos environments production+preview do EAS; perfis com `environment` explícito). M6 ✅ (submit iOS: appleId real, placeholders removidos — Team/App ID resolvem no submit interativo). A3 ✅ (.history/backend-api/ apagado, 79 snapshots). A2-handoffs ✅ (valores de R2/WEBHOOK substituídos por placeholders nos 2 handoffs do pipeline).
