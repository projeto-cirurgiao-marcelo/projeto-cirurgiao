...
function generateMockChallenges(): { daily: Challenge[]; weekly: Challenge[]; special: Challenge[] } {
  ...
  const daily: Challenge[] = [
    {
      id: 'ch-d1', type: 'daily', title: 'Assista 2 videoaulas',
      description: 'Complete pelo menos 2 videoaulas hoje', difficulty: 'easy',
      xpReward: 25, icon: 'Play', progress: 50, target: 2, current: 1,
      expiresAt: new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59).toISOString(),
      completedAt: null, claimedAt: null, status: 'active',
    },
    {
      id: 'ch-d2', type: 'daily', title: 'Passe em um quiz',
      description: 'Complete e passe em qualquer quiz disponível', difficulty: 'medium',
      xpReward: 50, icon: 'ClipboardCheck', progress: 0, target: 1, current: 0,
      expiresAt: new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59).toISOString(),
      completedAt: null, claimedAt: null, status: 'active',
    },
    {
      id: 'ch-d3', type: 'daily', title: 'Faça uma anotação',
      description: 'Crie uma anotação em qualquer videoaula', difficulty: 'easy',
      xpReward: 25, icon: 'NotebookPen', progress: 100, target: 1, current: 1,
      expiresAt: new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59).toISOString(),
      completedAt: new Date().toISOString(), claimedAt: null, status: 'completed',
    },
  ];

  const weekly: Challenge[] = [
    {
      id: 'ch-w1', type: 'weekly', title: 'Complete 10 videoaulas',
      description: 'Assista e complete 10 videoaulas esta semana', difficulty: 'hard',
      xpReward: 100, icon: 'Tv', progress: 30, target: 10, current: 3,
      expiresAt: new Date(now.getTime() + 5 * 24 * 60 * 60 * 1000).toISOString(),
      completedAt: null, claimedAt: null, status: 'active',
    },
    {
      id: 'ch-w2', type: 'weekly', title: 'Tire 90%+ em 3 quizzes',
      description: 'Alcance nota 90 ou superior em 3 quizzes diferentes', difficulty: 'hard',
      xpReward: 100, icon: 'Star', progress: 33, target: 3, current: 1,
      expiresAt: new Date(now.getTime() + 5 * 24 * 60 * 60 * 1000).toISOString(),
      completedAt: null, claimedAt: null, status: 'active',
    },
  ];
...
export const gamificationMock = {
  async getProfile(): Promise<GamificationProfileResponse> {
    ...
    return {
      ...
      streak: {
        current: 12, longest: 18,
        lastActiveDate: new Date().toISOString().split('T')[0],
        freezesAvailable: 1, todayCompleted: true,
      },
      stats: {
        videosCompleted: 23, totalWatchTimeMinutes: 1847,
        quizzesPassed: 8, quizAverageScore: 82,
        forumTopics: 5, forumReplies: 12, forumSolutions: 2,
        coursesCompleted: 1,
      },
  ...
  async getBadges(): Promise<BadgesResponse> {
    ...
    return { badges, summary: { total: badges.length, unlocked, byRarity } };
  },
  ...
  async getLeaderboard(
    ...
    const currentUserEntry: LeaderboardEntry = {
      rank: 7, userId: 'current-user', displayName: 'Você', avatarUrl: null,
      level: 5, levelTitle: 'Cirurgião Júnior', levelColor: '#F59E0B',
      xpEarned: period === 'weekly' ? 285 : 1200,
      videosCompleted: 6, quizzesPassed: 3, currentStreak: 12,
    };
    ...
    return {
      ...
      entries,
      currentUser: { rank: 7, xpEarned: currentUserEntry.xpEarned, level: 5, levelTitle: 'Cirurgião Júnior' },
    };
  },
  ...
  async getChallenges(): Promise<ChallengesResponse> {
    ...
    return { daily, weekly, special, completedToday: 1, totalCompleted: 24 };
  },

  async claimChallenge(_challengeId: string): Promise<ClaimChallengeResponse> {
    await delay(500);
    return { success: true, xpAwarded: 25, newTotalXp: 1875, levelUp: null, badgesUnlocked: [] };
  },
