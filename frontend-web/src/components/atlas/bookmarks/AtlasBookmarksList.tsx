...
interface AtlasBookmarksListProps {
  /** Default "Marcadores" */
  title?: string;
  count?: number;
  loading?: boolean;
  hint?: string;
  /** Quando renderizar como child de outro Card, passe noCard pra economizar wrapper */
  noCard?: boolean;
  compact?: boolean;
...
export function AtlasBookmarksList({
  title = "Marcadores",
  count,
  loading = false,
  hint,
  ...
  children,
  className,
}: AtlasBookmarksListProps) {
  const inner = (
    <>
      <AtlasCardHeader className={cn(compact && "px-4 py-3")}>
        <div className="flex items-center gap-2.5 min-w-0">
          <Bookmark
            className="size-4 text-atlas-warn shrink-0"
            strokeWidth={1.5}
          />
  ...
  return <AtlasCard className={className}>{inner}</AtlasCard>;
}
